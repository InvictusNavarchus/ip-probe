...
import { Clock, Globe, MapPin, Network, Server, Shield } from 'lucide-react';
import { useCurrentIP } from '../hooks/useIPAnalysis';
import { LoadingSpinner } from './LoadingSpinner';
import { Badge, ConfidenceBadge, RiskBadge, StatusBadge } from './ui/Badge';
      ...
      <Card className={className}>
        ...
        <CardContent>
          <div className="flex items-center justify-center py-8">
            ...
            <span className="ml-3 text-slate-600 dark:text-slate-400">Analyzing your connection...</span>
          </div>
        </CardContent>
      </Card>
    );
          ...
          {analysis.ip.primaryIP && (
            <div className="bg-slate-50 dark:bg-slate-700/50 rounded-lg p-4">
              <div className="flex items-center justify-between mb-3">
                <h3 className="font-semibold text-slate-900 dark:text-slate-100">Primary IP Address</h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div>
                    ...
                    <p className="font-medium text-slate-900 dark:text-slate-100">
                      ...
                      {analysis.ip.geolocation.countryCode && (
                        <span className="ml-2 text-sm text-slate-500">({analysis.ip.geolocation.countryCode})</span>
                      )}
                    </p>
                  </div>
                )}
                ...
                {analysis.ip.geolocation.region && (
                  <div>
                    ...
                    <p className="font-medium text-slate-900 dark:text-slate-100">{analysis.ip.geolocation.region}</p>
                  </div>
                )}
                ...
                {analysis.ip.geolocation.city && (
                  <div>
                    ...
                    <p className="font-medium text-slate-900 dark:text-slate-100">{analysis.ip.geolocation.city}</p>
                  </div>
                )}
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {analysis.ip.network.isp && (
                  <div>
                    ...
                    <p className="font-medium text-slate-900 dark:text-slate-100">{analysis.ip.network.isp}</p>
                  </div>
                )}
                ...
                {analysis.ip.network.organization && (
                  <div>
                    ...
                    <p className="font-medium text-slate-900 dark:text-slate-100">{analysis.ip.network.organization}</p>
                  </div>
                )}
                ...
                {analysis.ip.network.connectionType && (
                  <div>
                    ...
                    <Badge variant="info">{analysis.ip.network.connectionType}</Badge>
                  </div>
                )}
                  ...
                  <div>
                    ...
                    <p className="font-mono text-sm text-slate-900 dark:text-slate-100">
                      ...
                      {analysis.ip.network.asnOrganization && (
                        <span className="ml-2 font-sans">({analysis.ip.network.asnOrganization})</span>
                      )}
                    </p>
                  </div>
                )}
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                  {analysis.ip.security.isVPN !== undefined && (
                    <StatusBadge status={analysis.ip.security.isVPN ? 'warning' : 'online'} showDot={false}>
                      VPN: {analysis.ip.security.isVPN ? 'Yes' : 'No'}
                    </StatusBadge>
                  )}

                  {analysis.ip.security.isProxy !== undefined && (
                    <StatusBadge status={analysis.ip.security.isProxy ? 'warning' : 'online'} showDot={false}>
                      Proxy: {analysis.ip.security.isProxy ? 'Yes' : 'No'}
                    </StatusBadge>
                  )}

                  {analysis.ip.security.isTor !== undefined && (
                    <StatusBadge status={analysis.ip.security.isTor ? 'error' : 'online'} showDot={false}>
                      Tor: {analysis.ip.security.isTor ? 'Yes' : 'No'}
                    </StatusBadge>
                  )}

                  {analysis.ip.security.isHosting !== undefined && (
                    <StatusBadge status={analysis.ip.security.isHosting ? 'warning' : 'online'} showDot={false}>
